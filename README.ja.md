...
## Cloudflare Dashboard から GitHub 連携でデプロイする

リポジトリを GitHub に置き、Cloudflare ダッシュボードから接続してデプロイする方式。ローカルに Wrangler を入れたくない場合や、DevContainer 等で `wrangler login` が使いづらい環境向け。`wrangler.toml` はリポジトリに同梱しているため追加設定は不要。自分のリポジトリでない場合は fork してから接続する。

### 1. リポジトリを接続

1. **アプリケーションを作成** — Workers & Pages → アプリケーションを作成する → Connect to Git で GitHub を選択。
2. **リポジトリを選択** — GitHub アカウントを連携し、このリポジトリを選ぶ。
3. **ビルド設定** — Build command は空欄、Deploy command は `npx wrangler deploy`（デフォルトのまま）。
4. **デプロイ** — 初回ビルドが自動で走る。以降は `main` への push ごとに再デプロイされる。

### 2. KV Namespace を bind する

1. **Namespace を作成** — Storage & Databases → Workers KV → Create Instance（名前は任意）。
2. **Binding 編集画面を開く** — Worker → Settings → Bindings → Add → KV namespace。
3. **Bind する** — binding 名を `KV`、namespace は手順 1 のものを選ぶ。

### 3. 管理パスワードを設定

1. **変数編集画面を開く** — 同じ Worker → Settings → Variables and Secrets → Add。
2. **Secret を選択** — Type を **Secret**（Text ではない）にする。
3. **値を登録** — 変数名 `ADMIN_SECRET`、値は十分に長いランダム文字列。Save。

Worker の URL は `https://mcp-oauth-proxy.<your-subdomain>.workers.dev` のような形になる。
